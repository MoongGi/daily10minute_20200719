...
class ServerUtil
{
    ...
    companion object {
        ...
        //댓글 좋아요 선택
        fun postRequestLikeReply(context:Context, replyId: Int, handler: JsonResponseHandler?)
        {
            val client = OkHttpClient()

            //어떤 주소로 가야하는지 URL 을 후스트 주소 + 기능주소의 결합
            val urlString = "${BASE_URL}/like_proof_reply"
        ...
        //댓글 달기
        fun postRequestProofReply(context:Context, proofId: Int, content:String, handler: JsonResponseHandler?)
        {
            val client = OkHttpClient()

            //어떤 주소로 가야하는지 URL 을 후스트 주소 + 기능주소의 결합
            val urlString = "${BASE_URL}/proof_reply"
        ...
        //인증 등록
        fun postRequestProjectProof(context:Context, projectId: Int, content:String, handler: JsonResponseHandler?)
        {
            val client = OkHttpClient()

            //어떤 주소로 가야하는지 URL 을 후스트 주소 + 기능주소의 결합
            val urlString = "${BASE_URL}/project_proof"
...
//                    JSON내용 분석은 => 화면에서 진행하게 넘겨주자.
        ...
        //인증글 상세 조회
        fun getRequestProofDetail(context: Context, projectId: Int, handler: JsonResponseHandler?) {
...
//            GET / DELETE방식의 파라미터 첨부는 query에 담아야 함.
//            주소에 붙여주는 방식 => 쉽게 가공하도록 도와주는 변수 생성.
            val urlBuilder = "${BASE_URL}/project_proof/${projectId}".toHttpUrlOrNull()!!.newBuilder()
